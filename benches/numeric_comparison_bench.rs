...
fn benchmark_fixed_width_extremes(c: &mut Criterion) {
    let left = NumericValueRef::from(i128::MIN);
    let right = NumericValueRef::from(u128::MAX);
...
fn benchmark_big_integer(c: &mut Criterion) {
    ...
    group.bench_function("exact_bigint_vs_u128_max", |b| {
        b.iter(|| {
            let left = NumericValueRef::from(black_box(&integer));
            black_box(compare_numeric(
                black_box(left),
                black_box(right),
                black_box(policy),
    ...
    group.bench_function("exact_bigint_vs_bigint_equal", |b| {
        b.iter(|| {
            let left = NumericValueRef::from(black_box(&integer));
            let right = NumericValueRef::from(black_box(&same_integer));
            black_box(compare_numeric(
                black_box(left),
                black_box(right),
                black_box(policy),
...
fn benchmark_big_decimal(c: &mut Criterion) {
    ...
    group.bench_function("exact_bigdecimal_same_scale_equal", |b| {
        b.iter(|| {
            let left = NumericValueRef::from(black_box(&decimal));
            let right = NumericValueRef::from(black_box(&same_scale));
            black_box(compare_numeric(
                black_box(left),
                black_box(right),
                black_box(policy),
    ...
    group.bench_function("exact_bigdecimal_different_scale_equal", |b| {
        b.iter(|| {
            let left = NumericValueRef::from(black_box(&decimal));
            let right = NumericValueRef::from(black_box(&different_scale));
            black_box(compare_numeric(
                black_box(left),
                black_box(right),
                black_box(policy),
