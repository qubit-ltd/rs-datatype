...
fn test_duration_conversion_policy_defaults_preserve_semantics() {
    ...
    assert_eq!(
        policy.suffixless_string_policy(),
...
fn test_duration_conversion_policy_exact_wire_has_no_limits() {
    ...
    assert_eq!(
        serde_json::from_str::<DurationConversionPolicy>(&wire)
            .expect("duration policy should deserialize"),
        policy
    );
}
...
fn test_duration_conversion_policy_rejects_limit_fields() {
    let error = serde_json::from_str::<DurationConversionPolicy>(
        r#"{"max_text_bytes":8}"#,
    )
