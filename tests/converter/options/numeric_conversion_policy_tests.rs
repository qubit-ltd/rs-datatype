...
fn test_numeric_conversion_policy_profiles_preserve_semantics() {
    ...
    assert_eq!(strict.fractional_to_integer(), FractionalToIntegerPolicy::Reject);
    ...
    assert_eq!(lossy.fractional_to_integer(), FractionalToIntegerPolicy::Truncate);
    ...
    assert_eq!(env.fractional_to_integer(), FractionalToIntegerPolicy::Reject);
...
fn test_numeric_conversion_policy_exact_wire_has_no_limits() {
    ...
    assert_eq!(
        serde_json::from_str::<NumericConversionPolicy>(&wire).expect("numeric policy should deserialize"),
        policy
    );
}
