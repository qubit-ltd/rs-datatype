...
use qubit_datatype::{
    ConversionLimit, DataConversionOptions, DataConverter, DataType, DurationConversionOptions,
    DurationRoundingPolicy, DurationUnit, DurationUnitSuffixSet, InvalidValueReason,
    NumericConversionOptions, SuffixlessDurationPolicy,
};
...
fn test_data_converter_duration_text_honors_suffix_set() {
    let ascii = DataConversionOptions::strict().with_duration_options(
        DurationConversionOptions::default().with_unit_suffix_set(DurationUnitSuffixSet::Ascii),
    );
    assert_eq!(
