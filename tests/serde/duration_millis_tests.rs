...
fn test_duration_millis_deserialize_treats_integer_as_milliseconds() {
    let holder: DurationMillisHolder = serde_json::from_str(r#"{"duration":2}"#).expect("duration should deserialize");
...
fn test_duration_millis_serialize_rejects_first_rounded_overflow() {
    let duration = Duration::from_millis(u64::MAX)
        ...
        .expect("test duration should fit");
    let error =
        serde_json::to_string(&DurationMillisHolder { duration }).expect_err("rounded u64 overflow should fail");

    assert!(error.to_string().contains("duration exceeds u64 milliseconds"));
}
