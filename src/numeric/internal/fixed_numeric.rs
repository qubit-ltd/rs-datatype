...
pub(in crate::numeric) fn compare_magnitude(
    ...
    match left_high_bit.cmp(&right_high_bit) {
        Ordering::Equal => match left_exponent.cmp(&right_exponent) {
            ...
            Ordering::Greater => (left_significand << (left_exponent - right_exponent)).cmp(&right_significand),
            Ordering::Less => left_significand.cmp(&(right_significand << (right_exponent - left_exponent))),
        },
        ordering => ordering,
    }
}
